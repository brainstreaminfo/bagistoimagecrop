...
<script src="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.5.12/cropper.min.js"></script>
    ...
    >
        ...
        <div class="grid">
            ...
            <!--Image crop Modal -->
            <div ref="cropperModalWrapper" class="flex items-center justify-center bg-black bg-opacity-50 hidden">
                <div ref="cropperModal" class="bg-white p-4 rounded shadow-lg">
                    ...
                    <div class="flex justify-end mt-4">
                        <button
                            @click="closeCropper"
                            class="bg-info-500 hover:bg-info-700 text-white font-bold py-2 px-4 rounded mr-2"
                        >
                            @lang('imagescrop::app.imagecrop.cancel')
                        </button>
                        <button
                            @click="cropImage($event)"
                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                        >
                        @lang('imagescrop::app.imagecrop.crop')
                        </button>
                    </div>
                </div>
            </div>
    ...
    <script>
        window.translations = {
            cancel: "{{ trans('imagecrop::app.imagecrop.cancel') }}",
            crop: "{{ trans('imagecrop::app.imagecrop.crop') }}"
        };
    </script>
            ...
            <div ref="cropperModal" class="bg-white p-4 rounded shadow-lg">
                ...
                <div class="flex justify-end mt-4">
                    <button
                        ...
                        type="button" 
                    >
                        trans(imagescrop::app.imagecrop.cancel)
                    </button>
                    <button
                        @click="cropImage($event)"
                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                        type="button" 
                    >
                    trans(imagescrop::app.imagecrop.crop)
                    </button>
                </div>
            </div>
        </div>
            ...
            methods: {
                ...
                showCropper(file, index = null) {
                    ...
                    reader.onload = (e) => {
                        ...
                        cancelButton.className = 'bg-info-500 hover:bg-info-700 text-blue font-bold py-2 px-4 rounded mr-2';
                        cancelButton.textContent = window.translations.cancel;
                        cancelButton.type = 'button'; 
                        cancelButton.addEventListener('click', this.closeCropper);

                        const cropButton = document.createElement('button');
                        cropButton.className = 'bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded';
                        cropButton.textContent = window.translations.crop;
                        cropButton.type = 'button'; 
                        cropButton.addEventListener('click', () => this.cropImage(index));

                        buttonsContainer.append(cancelButton, cropButton);
                ...
                closeCropper() {
                    ...
                    this.cropper = null;
                    this.$refs.cropperModalWrapper.classList.add('hidden');
                },
            ...
            methods: {
                ...
                showCropper(file, index = null) {
                    ...
                    reader.onload = (e) => {
                        ...
                        const cancelButton = document.createElement('button');
                        cancelButton.classList.add('bg-info-500', 'hover:bg-info-700', 'text-black', 'font-bold', 'py-2', 'px-4', 'rounded', 'mr-2');
                        cancelButton.textContent = window.translations.cancel;
                        cancelButton.type = 'button'; // Ensure button type is 'button' to prevent default behavior
                        cancelButton.addEventListener('click', this.closeCropper);

                        const cropButton = document.createElement('button');
                        cropButton.classList.add('bg-blue-500', 'hover:bg-blue-700', 'text-white', 'font-bold', 'py-2', 'px-4', 'rounded');
                        cropButton.textContent = window.translations.crop;
                        cropButton.type = 'button'; // Ensure button type is 'button' to prevent default behavior
                        cropButton.addEventListener('click', () => this.cropImage(index));
                ...
                cropImage(index = null) {
                    if (event) {
                        event.preventDefault(); // Prevent page reload when cropping the image
                    }
                    ...
                    if (this.cropper) {
                        ...
                        canvas.toBlob((blob) => {
                            ...
                            this.closeCropper();
                        }, 'image/jpeg'); // Save as a JPEG, adjust as necessary
                    }
                },
